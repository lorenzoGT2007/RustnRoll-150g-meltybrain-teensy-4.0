...
const double minChosenMotorPulseWidth = 25.0;                    // in degrees, this value will be directly scaled as the RPM increases
const double maxChosenMotorPulseWidth = 180.0;                   // in degrees, can't be bigger than 180 degrees because the sinusoidal function would never reach the max rpm
...
void loop()
{
  ...
  else if (ch3Value > 0 && ch3Value <= 1000) // spin mode, the last else is for failsafe
  {
    ...
    motorThrottle= maxMotorThrottle(sin(angPos-calibratedDeg)+cos(motorPulseWidth/2))           with the conditions:
